...
describe('Campaigns', () => {
  ...
  it('allows a manager to make payment request', async () => {
    await campaign.methods
      .createRequest('Buy solarcell', '100', accounts[1])
      .send({ from: accounts[0], gas: '1000000' });

    const campaignRequest = await campaign.methods.requests(0).call();
    assert.equal(campaignRequest.description, 'Buy solarcell');
  });

  it('processes requests', async () => {
    // get last amount of balance from accounts[1]
